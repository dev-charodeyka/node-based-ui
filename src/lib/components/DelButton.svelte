<script lang="ts">
  import { handleDelVertex } from '$lib/utils/delAddActions.svelte';
</script>

<button
  onclick={handleDelVertex}
  class="text-aura-purple [&:hover>svg>path:first-child]:text-aura-yellow [&:hover>svg>path:first-child]:stroke-aura-yellow [&:hover>svg>g>path]:text-dark-purple absolute right-1 top-1 h-7 cursor-pointer rounded-full"
  ><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24"
    ><!-- Icon from Myna UI Icons by Praveen Juge - https://github.com/praveenjuge/mynaui-icons/blob/main/LICENSE -->
    <path
      class="group-hover:animate-rotate text-vertex-info-bg stroke-aura-purple origin-center fill-current"
    ...
    <g transform="translate(12 12) scale(0.6) translate(-12 -12)">
      <!-- Icon from Myna UI Icons by Praveen Juge - https://github.com/praveenjuge/mynaui-icons/blob/main/LICENSE -->
      <path
        class="stroke-aura-purple fill-current"
