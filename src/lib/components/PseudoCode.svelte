...
<div class="grid h-full w-full grid-rows-10 text-xs">
  <div class="justify-left flex h-full w-full items-center text-sm">
    ...
    <span class="bg-aura-darkpurple rounded-md px-1 py-0.5">{algInputPlaceholder()}</span>
    <span>{'):'}</span>
  </div>
  {#each traversedPath.slice(1) as step, i (step)}
    <div class="justify-left flex h-full w-full items-center pl-6 text-sm">
      <span>{vertices.find((vertex) => vertex.id === step)?.output || 'output'}</span>
      <span class="bg-aura-darkpurple rounded-md px-1 py-0.5"
        >{vertices.find((vertex) => vertex.id === step)?.codeName || 'operation'}</span
      >
      <span>{'('}</span>
      <span>{vertices.find((vertex) => vertex.id === traversedPath[i])?.output || 'Input'}</span>
      <span>{')'}</span>
    </div>
  {/each}
